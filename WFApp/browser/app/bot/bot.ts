...
import { addAudioVolumeControlToStream } from './volume';
...
export class WorkoutBotApp implements AudioVideoObserver, DeviceChangeObserver, ContentShareObserver {
  static readonly BASE_URL: string = [location.protocol, '//', location.host, location.pathname.replace(/\/*$/, '/').replace('/bot', '')].join('');
  ...
  static readonly SIGNAL_MESSAGE_LIFETIME_MS: number = 2000;
  static readonly DataMessageTopicJoinClass: string = "Join";
  ...
  workoutId: string = '0';
  bVirtualCoachWorkout: boolean = false;
  bWithoutCoach: boolean = false;
  bShowingRecordedCoachVideo: boolean = false;
  bTrainerJoined: boolean = false;
  ...
  endTime: any;
  videoIndex: number = 0;
  nextVideoElementIndex: number = -1;
  videoElementIndex: number = -1;
  videoCanvasContext: any = null;
  videoCanvas: HTMLCanvasElement = null;
  isWorkoutPaused: boolean = false;
  isWorkoutOver: boolean = false;
  contentStream: MediaStream = null;
  nextVideoTimeoutHandler:TimeoutScheduler = null;
  mainProcessTimeoutHandler:TimeoutScheduler = null;
  res_path = "";
  videoInfoList: any;
  videoList: any = [];
  videoInfo: any;
  ...
  jsPlayFlag = false;
  curVideoPos: number = 0;
  bStatusSwooshPlaying = false;
  ...
  welcomeTime = 0;
  bVideoPaused = true;
  bShowingNextVideoClip = false;
  bTimerActivated = false;
  bSwitchActivated = false;
  bPlayingCoachPreviewAudio = false;
  ...
  songUrl: any = null;
  nextMusicElementIndex: number = -1;
  musicElementIndex: number = -1;
  ...
  coachVideoVolume: number = 1;
  memberMusicVolume: number = 0.7;
  ...
  async updatePulseData(): Promise<void> {
    try {
      const response = await fetch(
        this.api_base_url + `Home/GetCustomerPulseByClassScheduleId?ClassScheduleId=${encodeURIComponent(this.classScheduleId)}`,
        {
          method: 'POST',
          mode: 'cors'
        }
...
//      console.log("pulse data:"+JSON.stringify(json));
    ...
    new TimeoutScheduler(2000).start( async () => {
  ...
  async setupEventReporter(configuration: MeetingSessionConfiguration): Promise<EventReporter> {
    ...
    if (this.isLocalHost()) {
      ...
      const eventReportingPOSTLogger = getPOSTLogger(configuration, 'SDKEventIngestion', `${WorkoutBotApp.BASE_URL}log_event_ingestion`, LogLevel.DEBUG);
...
//    this.lrSignCanvas.crossOrigin ="anonymous";
  ...
  //  this.alSignCanvas.crossOrigin ="anonymous";
    ...
    this.videoCanvas = document.getElementById('trainVideoCanvas') as HTMLCanvasElement;
    this.videoCanvas.width = this.cw;
    this.videoCanvas.height = this.ch;
    this.videoCanvasContext = this.videoCanvas.getContext('2d');

    this.workoutVideoElement = document.getElementById('content-share-video') as HTMLVideoElement;
    this.workoutNextVideoElement = document.getElementById('content-share-next-video') as HTMLVideoElement;

    this.coachVideoElement1 = document.getElementById('CoachVideo') as HTMLVideoElement;
    this.coachVideoElement2 = document.getElementById('NextCoachVideo') as HTMLVideoElement;

    this.coachPreviewAudioElement = document.getElementById('CoachPreviewAudio') as HTMLVideoElement;

    this.timerVideo = document.getElementById('TimerVideo') as HTMLVideoElement;
    this.timerVideoForStretch = document.getElementById('TimerVideoForStretch') as HTMLVideoElement;
    this.spinVideo = document.getElementById('SpinVideo') as HTMLVideoElement;
    this.spinVideoForStretch = document.getElementById('SpinVideoForStretch') as HTMLVideoElement;
    this.audioBoxingBell = document.getElementById('BoxingBellAudio') as HTMLAudioElement;
    this.audioBoxingBell6 = document.getElementById('BoxingBellAudio6') as HTMLAudioElement;
    this.audioSwooshLong = document.getElementById('SwooshLongAudio') as HTMLAudioElement;
    this.audioSwooshShort = document.getElementById('SwooshShortAudio') as HTMLAudioElement;
    this.audioWatchTick = document.getElementById('WatchTickAudio') as HTMLAudioElement;
    this.audioBlip = document.getElementById('BlipAudio') as HTMLAudioElement;
    this.audioBong = document.getElementById('BongAudio') as HTMLAudioElement;
    this.audioStretchGong = document.getElementById('StretchGongAudio') as HTMLAudioElement;
    this.audioStretchThump = document.getElementById('StretchThumpAudio') as HTMLAudioElement;

    this.musicElement1 = document.getElementById('Music1') as HTMLAudioElement;
    this.musicElement2 = document.getElementById('Music2') as HTMLAudioElement;
  ...
  prepareImageSign(sbc: any,img: any,signWidth: any){
  ...
  mainProcess(){
    ...
    if ( !this.isWorkoutOver ){
      this.mainProcessTimeoutHandler = new TimeoutScheduler(30);
      this.mainProcessTimeoutHandler.start( async () => {
        this.mainProcess();
      });
    } else {
      if ( this.currentMusicElement && !this.currentMusicElement.paused && !this.currentMusicElement.ended )
  ...
  renderNextVideoTitle(c: any,w: any) {
...
//      console.log('here '+textToDisplay);
  ...
  drawImageOnCanvas( img: any, bCoachArea?: any){
  ...
  drawNextVideoClip(c: any, w: any, h: any, nv: any){
...
//        `${WorkoutBotApp.BASE_URL}bothandler?botAction=stop&taskId=${this.taskId}`,
  ...
  startProcess(): void {
    new AsyncScheduler().start(async () => {
      ...
      if ( this.startTime.getTime() - now.getTime()>=5000){
        ...
        new TimeoutScheduler(this.startTime.getTime() - now.getTime()).start( async () => {
          this.timeLeft = 0;
        ...
        new TimeoutScheduler(5000).start( async () => {
          this.timeLeft = 0;
          this.startPlayingVideo(); //it should call exactly at workout start time.
        });
      }
      ...
      if ( threeMinBeforeEndTime.getTime() - now.getTime()>=0){
        new TimeoutScheduler(threeMinBeforeEndTime.getTime() - now.getTime()).start( async () => {
          let jsonMsg = {
            "id": "threeMinLeft"
          };
          this.audioVideo.realtimeSendDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_VIDEO_STATUS, JSON.stringify(jsonMsg),WorkoutBotApp.SIGNAL_MESSAGE_LIFETIME_MS);           
  ...
  async createLogStream(configuration: MeetingSessionConfiguration, pathname: string): Promise<void> {
    ...
    try {
      const response = await fetch(`${WorkoutBotApp.BASE_URL}${pathname}`, {
        method: 'POST',
        body
      });
      if (response.status === 200) {
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
      ...
      this.meetingSessionPOSTLogger = getPOSTLogger(configuration, 'SDK', `${WorkoutBotApp.BASE_URL}logs`, LogLevel.INFO);
      logger = new MultiLogger(
          consoleLogger,
          this.meetingSessionPOSTLogger,
      );
      this.meetingEventPOSTLogger = getPOSTLogger(configuration, 'SDKEvent', `${WorkoutBotApp.BASE_URL}log_meeting_event`, LogLevel.INFO);
    }
    ...
    this.meetingSession = new DefaultMeetingSession(
      ...
      logger,
      deviceController,
      new DefaultEventController(configuration, logger, this.eventReporter)
    );
...
//        this.bTrainerJoined = present;
  ...
  setupDataMessage(): void {
    ...
    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC, (dataMessage: DataMessage) => {
      this.dataMessageHandler(dataMessage);
    });

    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_ONE_BY_ONE_CHAT_STATUS, (dataMessage: DataMessage) => {
    ...
    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_VIDEO_PAUSE, async (dataMessage: DataMessage) => {
     // const videoFile = document.getElementById('content-share-video') as HTMLVideoElement;
      ...
      if(dataMessage.text() == "Pause"){
        ...
        if ( this.bWithoutCoach ){
          if ( this.currentCoachVideoElement.readyState > 0 ){
              await this.currentCoachVideoElement.play();
          }
          if ( this.bPlayingCoachPreviewAudio && this.coachPreviewAudioElement.readyState > 0 )
            await this.coachPreviewAudioElement.play();
        }   
        if ( this.bTimerActivated ){
          if ( this.videoInfo.Stretch == 1 ){
            await this.timerVideoForStretch.play();
            currentTime = this.timerVideoForStretch.currentTime;
          }
          else{
            await this.timerVideo.play();
            currentTime = this.timerVideo.currentTime;
          }
        }
      } else {
        ...
        if (jsonData.id == "Play") {
          ...
          await this.audioVideo.realtimeSendDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_VIDEO_STATUS, JSON.stringify(jsonMsg),WorkoutBotApp.SIGNAL_MESSAGE_LIFETIME_MS);    
          ...
          new TimeoutScheduler(1000).start( async ()=>{
            this.isWorkoutOver = false; 
    ...
    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_GET_VIDEO_STATUS, async (dataMessage: DataMessage) => {
      let currentTime = 0;
      if ( !this.videoInfo )
        currentTime = 0;
    ...
    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_VIDEO_STATUS, (dataMessage: DataMessage) => {
      if (dataMessage.text() != null && dataMessage.text() != '') {
        const jsonData = JSON.parse(dataMessage.text());
      	console.log(WorkoutBotApp.DATA_MESSAGE_TOPIC_VIDEO_STATUS+':'+dataMessage.text());
        if (jsonData.id == "RecordingVideoStatus") {
          this.recordingFlag = jsonData.recordingFlag;
        }
      }
    });

    this.audioVideo.realtimeSubscribeToReceiveDataMessage(WorkoutBotApp.DATA_MESSAGE_TOPIC_MEMBER_MUSIC_STATUS, (dataMessage: DataMessage) => {
      if (dataMessage.text() != null && dataMessage.text() != '') {
        const jsonData = JSON.parse(dataMessage.text());
...
//            (window as any).setContentVolume(this.memberMusicVolume);
  ...
  async joinMeeting(): Promise<any> {
    const response = await fetch(
      `${WorkoutBotApp.BASE_URL}join?title=${encodeURIComponent(this.meeting)}&name=${encodeURIComponent(this.name)}&region=${encodeURIComponent(this.region)}&role=${encodeURIComponent(this.role)}&customerId=${encodeURIComponent(this.customerId)}`,
      {
        method: 'POST',
      }
    );
  ...
  async endMeeting(): Promise<any> {
    await fetch(`${WorkoutBotApp.BASE_URL}end?title=${encodeURIComponent(this.meeting)}`, {
      method: 'POST',
    });
  }
    ...
    switch (this.contentShareType) {
      ...
      case ContentShareType.CanvasCapture:
        console.log('starting share video:');
        {
            ...
            const { stream, setVolume } = addAudioVolumeControlToStream(this.combinedStream);
            (window as any).setContentVolume = setVolume;
